...
const malefica = {
  ...
  villainDeck: [
    ...
    { id: 'mal_c_son_1', name: 'Sonno Senza Sogni', type: 'curse', cost: 3,
      effect: 'Gli Eroi in questo luogo hanno Forza -2. Scarta questa Maledizione se un Alleato viene giocato direttamente in questo luogo (non mosso). Il Corvo mosso qui NON la scarta. Solo Fauna (mazzo Fato) può rimuoverla tramite la sua abilità speciale.' },
    ...
    { id: 'mal_c_fuo_1', name: 'Fuoco Verde', type: 'curse', cost: 3,
      effect: 'Gli Eroi non possono essere giocati in questo luogo. Malefica non può muoversi in questo luogo. Scarta questa Maledizione se Malefica viene spostata qui (es. per effetto di Re Stefano). Il Corvo e Svanire aggirano comunque le azioni del luogo.' },
    ...
    { id: 'mal_a_cor', name: 'Corvo', type: 'ally', cost: 3, strength: 3,
      effect: 'Prima che Malefica si muova, puoi usare un\'azione disponibile (non Fato) nel luogo in cui si trova il Corvo. Se Malefica non si muove (es. per Svanire), il Corvo può comunque spostarsi. Non può usare azioni coperte da Eroi.' },
    ...
    { id: 'mal_a_sel_3', name: 'Scagnozzo Selvaggio', type: 'ally', cost: 4, strength: 5,
      effect: 'Nessun effetto speciale.' },
    { id: 'mal_a_sin_1', name: 'Scagnozzo Sinistro', type: 'ally', cost: 3, strength: 3,
      effect: 'Ottiene +1 Forza per ogni Maledizione nel suo luogo. Non giocarlo in un luogo con Sonno Senza Sogni (lo scarterebbe); preferibilmente spostacelo dopo.' },
    { id: 'mal_a_sin_2', name: 'Scagnozzo Sinistro', type: 'ally', cost: 3, strength: 3,
      effect: 'Ottiene +1 Forza per ogni Maledizione nel suo luogo. Non giocarlo in un luogo con Sonno Senza Sogni (lo scarterebbe); preferibilmente spostacelo dopo.' },
    { id: 'mal_a_sin_3', name: 'Scagnozzo Sinistro', type: 'ally', cost: 3, strength: 3,
      effect: 'Ottiene +1 Forza per ogni Maledizione nel suo luogo. Non giocarlo in un luogo con Sonno Senza Sogni (lo scarterebbe); preferibilmente spostacelo dopo.' },
    // OGGETTI (2)
    { id: 'mal_o_arc', name: 'Arcolaio', type: 'item', cost: 1,
      effect: 'Quando un Eroe in questo luogo viene sconfitto, guadagna Potere pari alla Forza attuale di quell\'Eroe meno 1 (tiene conto dei malus in corso, es. Sonno Senza Sogni).' },
    ...
    { id: 'mal_e_dra_1', name: 'Forma di Drago', type: 'effect', cost: 3,
      effect: 'Sconfiggi qualsiasi Eroe con Forza ≤3 in questo luogo. Poi, guadagna 3 Potere per ogni azione Fato scagliata contro di te prima del tuo prossimo turno.' },
    ...
    { id: 'mal_e_sva_1', name: 'Svanire', type: 'effect', cost: 2,
      effect: 'Al tuo prossimo turno, Malefica non è obbligata a spostarsi in un nuovo luogo. Può rimanere dove si trova e usare le azioni di quel luogo come se vi fosse appena arrivata.' },
    ...
    { id: 'mal_k_tir_1', name: 'Tirannia', type: 'condition', cost: 0,
      effect: 'Attiva durante il turno di un avversario che ha 3 o più Alleati nel suo Reame. Effetto: pesca 3 carte nella tua mano, poi scarta 3 carte a tua scelta.' },
    { id: 'mal_k_tir_2', name: 'Tirannia', type: 'condition', cost: 0,
      effect: 'Attiva durante il turno di un avversario che ha 3 o più Alleati nel suo Reame. Effetto: pesca 3 carte nella tua mano, poi scarta 3 carte a tua scelta.' },
    { id: 'mal_k_mal_1', name: 'Malignità', type: 'condition', cost: 0,
      effect: 'Attiva durante il turno di un avversario che ha 3 o più Eroi nel suo Reame. Effetto: sconfiggi un Eroe con Forza ≤4 in qualsiasi luogo del tuo Reame.' },
  ...
  fateDeck: [
    ...
    { id: 'fmal_aurora',   name: 'Aurora',           type: 'hero', strength: 4,
      effect: 'Quando viene giocata: scarta la Foresta di Rovi nel luogo in cui viene giocata (se presente). Poi rivela la prima carta del mazzo Fato: se è un Eroe, giocalo in qualsiasi luogo; altrimenti rimettila in cima al mazzo.' },
    { id: 'fmal_fauna',    name: 'Fauna',             type: 'hero', strength: 2,
      effect: 'Quando viene giocata: puoi scartare un Sonno Senza Sogni dal luogo in cui viene giocata. Se ci sono più copie, ne viene scartata solo una.' },
    { id: 'fmal_flora',    name: 'Flora',             type: 'hero', strength: 3,
      effect: 'Mentre è in campo: Malefica gioca con la mano scoperta (tutti i giocatori possono vedere le sue carte in mano). Finché Flora è in campo, Malefica deve giocare con le carte rivelate.' },
    { id: 'fmal_serena',   name: 'Serena',            type: 'hero', strength: 4,
      effect: 'Mentre è in campo: le Maledizioni non possono essere giocate nel luogo in cui si trova Serena. Malefica può però spostare una Maledizione già esistente in quel luogo.' },
    { id: 'fmal_stefano',  name: 'Re Stefano',        type: 'hero', strength: 4,
      effect: 'Quando viene giocato: scarta la Foresta di Rovi presente in questo luogo (se presente). Poi sposta Malefica in qualsiasi luogo del Reame. Se Malefica viene spostata in un luogo con Fuoco Verde, quella Maledizione viene scartata.' },
    { id: 'fmal_uberto',   name: 'Re Uberto',         type: 'hero', strength: 3,
      effect: 'Quando viene giocato: puoi spostare un Alleato da ogni luogo adiacente al luogo di Re Uberto, portandolo nel luogo di Re Uberto.' },
    { id: 'fmal_filippo',  name: 'Principe Filippo',  type: 'hero', strength: 5,
      effect: 'Quando viene giocato: puoi scartare tutti gli Alleati presenti in ogni luogo adiacente al suo luogo (incluso il Corvo). Bersaglio ideale: il Corvo.' },
    { id: 'fmal_guard_1',  name: 'Guardie',           type: 'hero', strength: 3,
      effect: 'Per essere sconfitte con un\'azione Scontro servono almeno 2 Alleati. Forma di Drago le sconfigge normalmente.' },
    ...
    { id: 'fmal_spada_1',  name: 'Spada della Verità', type: 'fate_item',
      effect: 'Assegna a un Eroe privo di altri oggetti assegnati. Quell\'Eroe ottiene +2 Forza e il costo per giocare una Maledizione nel suo luogo aumenta di 2 Potere.' },
    ...
    { id: 'fmal_sogno_1',  name: 'C\'era una Volta in un Sogno', type: 'fate_effect',
      effect: 'Scegli un luogo del Reame di Malefica che contiene almeno un Eroe e almeno una Maledizione. Scarta quella Maledizione.' },
...
const hook = {
  ...
  villainDeck: [
    ...
    { id: 'hk_a_sma_3', name: 'Smargiasso', type: 'ally', cost: 1, strength: 2,
      effect: 'Nessun effetto speciale.' },
    { id: 'hk_a_bru_1', name: 'Bruto Pirata', type: 'ally', cost: 4, strength: 4,
      effect: 'Nessun effetto speciale.' },
    { id: 'hk_a_bru_2', name: 'Bruto Pirata', type: 'ally', cost: 4, strength: 4,
      effect: 'Nessun effetto speciale.' },
    { id: 'hk_a_sta', name: 'Mr. Starkey', type: 'ally', cost: 2, strength: 2,
      effect: 'Quando viene giocato: puoi spostare un Eroe dal suo luogo in un luogo adiacente sbloccato.' },
    ...
    { id: 'hk_e_spa_1', name: 'Spaventare', type: 'effect', cost: 2,
      effect: 'Guarda le prime 2 carte del tuo mazzo Fato. Puoi scartarle entrambe o rimetterle in cima nell\'ordine che vuoi.' },
    ...
    { id: 'hk_e_deg_1', name: 'Degno Avversario', type: 'effect', cost: 0,
      effect: 'Guadagna 3 Potere. Rivela carte dalla cima del tuo mazzo Fato finché non trovi un Eroe. Giocalo nel luogo che preferisci e scarta le restanti.' },
    ...
    { id: 'hk_e_sig_1', name: 'Signore, Signorsì Signore!', type: 'effect', cost: 2,
      effect: 'Sposta un Alleato in un luogo adiacente sbloccato. Quell\'Alleato ottiene +2 Forza fino alla fine del turno.' },
    ...
    { id: 'hk_k_oss_1', name: 'Ossessione', type: 'condition', cost: 0,
      effect: 'Attiva quando: un avversario sconfigge un Eroe con Forza ≥4. Effetto: rivela carte dalla cima del tuo mazzo Fato finché non trovi un Eroe. Puoi giocarlo o scartarlo. Scarta le restanti.' },
  ...
  fateDeck: [
    ...
    { id: 'fhk_peter', name: 'Peter Pan', type: 'hero', strength: 7,
      effect: 'Quando viene rivelato durante un Fato: deve essere immediatamente giocato all\'Albero dell\'Impiccato (anche se bloccato); le altre carte Fato rivelate vengono scartate. Deve essere sconfitto alla Jolly Roger per far vincere Uncino.' },
    { id: 'fhk_gianni', name: 'Gianni', type: 'hero', strength: 1,
      effect: 'Ottiene +1 Forza se ha almeno un Oggetto assegnato (non +1 per ogni Oggetto).' },
    ...
    { id: 'fhk_michele', name: 'Michele', type: 'hero', strength: 1,
      effect: 'Ottiene +1 Forza per ogni luogo del Reame di Uncino che contiene almeno un Eroe (incluso il suo), per un totale potenziale di +4.' },
    { id: 'fhk_tictac', name: 'Tic Tac', type: 'hero', strength: 5,
      effect: 'Mentre è nel Reame: se Capitan Uncino si sposta nel suo luogo, deve scartare immediatamente tutta la sua mano.' },
    { id: 'fhk_trilli', name: 'Trilli', type: 'hero', strength: 2,
      effect: 'Quando viene giocata: puoi scartare un Alleato (a tua scelta) dal suo luogo.' },
    { id: 'fhk_wendy', name: 'Wendy', type: 'hero', strength: 3,
      effect: 'Mentre è in campo: tutti gli altri Eroi nel Reame di Uncino ottengono +1 Forza.' },
    // OGGETTI FATO (5)
    { id: 'fhk_polv_1', name: 'Polvere di Fata', type: 'fate_item',
      effect: 'Assegna a un Eroe. Quell\'Eroe ottiene +2 Forza. Non può essere assegnata a Peter Pan se l\'Albero dell\'Impiccato è ancora bloccato.' },
    ...
    { id: 'fhk_sch_1', name: 'Schernire', type: 'fate_item',
      effect: 'Assegna a un Eroe. Uncino deve sconfiggere un Eroe con Schernire prima di poterne sconfiggere altri. Se più Eroi hanno Schernire, Uncino sceglie quale dei due sconfiggere per primo.' },
    { id: 'fhk_sch_2', name: 'Schernire', type: 'fate_item',
      effect: 'Assegna a un Eroe. Uncino deve sconfiggere un Eroe con Schernire prima di poterne sconfiggere altri. Se più Eroi hanno Schernire, Uncino sceglie quale dei due sconfiggere per primo.' },
    // EFFETTI FATO (2)
    { id: 'fhk_mal_1', name: 'Terribile Mal di Testa', type: 'fate_effect',
      effect: 'Scarta un Oggetto dal Reame di Capitan Uncino (preferibilmente Dispositivo Ingegnoso o Cannone).' },
...
const ursula = {
  ...
  villainDeck: [
    ...
    { id: 'urs_o_cor', name: 'Corona', type: 'item', cost: 4,
      effect: 'Condizione di vittoria: deve trovarsi al Covo di Ursula. Attivazione: guarda la prima carta del tuo mazzo Fato; rimettila in cima o in fondo al mazzo.' },
    { id: 'urs_o_tri', name: 'Tridente', type: 'item', cost: 4,
      effect: 'Condizione di vittoria: deve trovarsi al Covo di Ursula. Quando viene giocato: trova Re Tritone (nel mazzo, nello scarto o già in campo) e posizionalo nel luogo del Tridente; se era in campo tutti i suoi oggetti assegnati vengono scartati.' },
    { id: 'urs_o_cal', name: 'Calderone', type: 'item', cost: 1,
      effect: 'Attivazione: ottieni 2 Potere per ogni Contratto Vincolante nel tuo Reame.' },
    // ALLEATI (2)
    { id: 'urs_a_flo', name: 'Flotsam', type: 'ally', cost: 2, strength: 2,
      effect: 'Attivazione: muovi un Eroe dal luogo in cui si trova Flotsam in un luogo adiacente non bloccato.' },
    { id: 'urs_a_jet', name: 'Jetsam', type: 'ally', cost: 2, strength: 2,
      effect: 'Attivazione: muovi un Eroe dal luogo in cui si trova Jetsam in un luogo adiacente non bloccato.' },
    // EFFETTI (14)
    { id: 'urs_e_tra_1', name: 'Trasformazione', type: 'effect', cost: 1,
      effect: 'Sposta il lucchetto: se il Palazzo è bloccato, sbloccalo e blocca il Covo (e viceversa).' },
    ...
    { id: 'urs_e_gig_1', name: 'Diventare Gigantesca', type: 'effect', cost: 1,
      effect: 'Esegui un\'azione di qualsiasi luogo adiacente a quello in cui si trova Ursula, anche se quel luogo è bloccato (incluse azioni Fato).' },
    { id: 'urs_e_gig_2', name: 'Diventare Gigantesca', type: 'effect', cost: 1,
      effect: 'Esegui un\'azione di qualsiasi luogo adiacente a quello in cui si trova Ursula, anche se quel luogo è bloccato (incluse azioni Fato).' },
    { id: 'urs_e_gig_3', name: 'Diventare Gigantesca', type: 'effect', cost: 1,
      effect: 'Esegui un\'azione di qualsiasi luogo adiacente a quello in cui si trova Ursula, anche se quel luogo è bloccato (incluse azioni Fato).' },
    { id: 'urs_e_opp_1', name: 'Opportunista', type: 'effect', cost: 0,
      effect: 'Scegli un Oggetto o un Effetto dalla tua pila degli scarti e aggiungilo alla tua mano.' },
    { id: 'urs_e_opp_2', name: 'Opportunista', type: 'effect', cost: 0,
      effect: 'Scegli un Oggetto o un Effetto dalla tua pila degli scarti e aggiungilo alla tua mano.' },
    { id: 'urs_e_opp_3', name: 'Opportunista', type: 'effect', cost: 0,
      effect: 'Scegli un Oggetto o un Effetto dalla tua pila degli scarti e aggiungilo alla tua mano.' },
    { id: 'urs_e_vor_1', name: 'Vortice', type: 'effect', cost: 2,
      effect: 'Sposta un Eroe dal Reame di Ursula in un luogo non bloccato a tua scelta.' },
    ...
    { id: 'urs_e_div_1', name: 'Divinazione', type: 'effect', cost: 1,
      effect: 'Rivela carte dal tuo mazzo finché non trovi un Contratto Vincolante; mettilo in mano o scartalo. Scarta le altre carte rivelate.' },
    { id: 'urs_e_div_2', name: 'Divinazione', type: 'effect', cost: 1,
      effect: 'Rivela carte dal tuo mazzo finché non trovi un Contratto Vincolante; mettilo in mano o scartalo. Scarta le altre carte rivelate.' },
    { id: 'urs_e_tri', name: 'Tristi Anime Sole', type: 'effect', cost: 3,
      effect: 'Sposta ogni Eroe nel tuo Reame in un luogo adiacente non bloccato a tua scelta.' },
    // CONDIZIONI (4)
    { id: 'urs_k_arr_1', name: 'Arroganza', type: 'condition', cost: 0,
      effect: 'Attiva quando: un avversario sconfigge un Eroe con Forza ≥4. Effetto: pesca 3 carte dal tuo mazzo e aggiungile alla tua mano.' },
    { id: 'urs_k_arr_2', name: 'Arroganza', type: 'condition', cost: 0,
      effect: 'Attiva quando: un avversario sconfigge un Eroe con Forza ≥4. Effetto: pesca 3 carte dal tuo mazzo e aggiungile alla tua mano.' },
    { id: 'urs_k_ing_1', name: 'Inganno', type: 'condition', cost: 0,
      effect: 'Attiva quando: un avversario guadagna Potere. Effetto: rivela e gioca la prima carta del mazzo Fato di quell\'avversario (come se stessi eseguendo un Fato contro di lui).' },
  ...
  fateDeck: [
    ...
    { id: 'furs_ariel',    name: 'Ariel',      type: 'hero', strength: 4,
      effect: 'Quando viene giocata: puoi spostare un Oggetto non assegnato a un Eroe da un luogo non bloccato a un luogo non bloccato a tua scelta. Mentre Ariel è in campo, Ursula non può usare l\'azione Gioca una carta.' },
    { id: 'furs_eric',     name: 'Eric',       type: 'hero', strength: 4,
      effect: 'Quando viene giocato: puoi spostare un Eroe del Reame di Ursula in qualsiasi luogo non bloccato.' },
    { id: 'furs_flounder', name: 'Flounder',   type: 'hero', strength: 1,
      effect: 'Quando viene giocato: rimischia la pila degli scarti villain di Ursula nel suo mazzo.' },
    { id: 'furs_grimsby',  name: 'Grimsby',    type: 'hero', strength: 2,
      effect: 'Quando viene giocato: il giocatore che usa il Fato sceglie se spostare il lucchetto al Covo di Ursula oppure al Palazzo.' },
    { id: 'furs_tritone',  name: 'Re Tritone',  type: 'hero', strength: 6,
      effect: 'I Contratti Vincolanti assegnati a lui e ogni carta che lo ha come obiettivo (es. Vortice, Tristi Anime Sole) costano 1 Potere in più. Quando il Tridente viene giocato: viene spostato nel luogo del Tridente (tutti i suoi oggetti assegnati vengono scartati).' },
    { id: 'furs_max',      name: 'Max',        type: 'hero', strength: 3,
      effect: 'Quando viene giocato: sposta Ursula in qualsiasi luogo non bloccato del suo Reame.' },
    { id: 'furs_scuttle',  name: 'Scuttle',    type: 'hero', strength: 2,
      effect: 'Quando viene giocato: puoi scegliere un Oggetto Fato dalla pila degli scarti di Ursula, giocarlo nel luogo corrente e assegnarlo a Scuttle.' },
    { id: 'furs_sebas',    name: 'Sebastian',  type: 'hero', strength: 2,
      effect: 'Quando viene giocato: puoi scegliere un Contratto Vincolante assegnato a un Eroe in un luogo non bloccato e riassegnarlo a Sebastian.' },
    // OGGETTI FATO (4)
    ...
    { id: 'furs_rip_1', name: 'Riprendere Forma', type: 'fate_effect',
      effect: 'Scegli un Eroe con Forza ≤4 dalla pila degli scarti del Mazzo Fato di Ursula. Giocalo nel luogo in cui si trova Ursula.' },
